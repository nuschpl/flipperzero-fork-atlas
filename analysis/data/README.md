...
- Then: `node analysis/build.mjs` and commit `features.json`, `structure.json`,
...
- **Documents tab** needs HTTP (browsers block `file://` fetch):
  ```
  python3 -m http.server 8771 --directory analysis
  # → http://localhost:8771/feature-explorer.html
  ```
